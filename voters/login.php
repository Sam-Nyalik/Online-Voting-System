...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($emailAddress_error) && empty($password_error)) {
        ...
        $sql = "SELECT voterId, emailAddress, password FROM voters WHERE emailAddress = :emailAddress";
        if ($stmt = $pdo->prepare($sql)) {
            ...
            // Attempt to execute
            if ($stmt->execute()) {
                // Check if the user with the input emailAddress exists
                if ($stmt->rowCount() == 1) {
                    //User exists
                    if ($row = $stmt->fetch()) {
                        $id = $row['voterId'];
                        ...
                        if (password_verify($password, $hashed_password)) {
                            ...
                            header("location: index.php?page=voters/dashboard");
                            exit;
                        } else {
...
<div class="breadcrumb">
    <div class="container">
        <div class="row">
            <span><a href="index.php?page=home">Home</a> > Voter Login</span>
        </div>
    </div>
</div>
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-5">
                <!-- Title -->
                <h3>Voter Login</h5>
                    <p>Login to cast your vote today</p>
                    <hr>

                    <!-- Login Form -->
                    <form action="index.php?page=voters/login" method="post" class="login-form">
                        <!-- Email Address -->
                        ...
                        <!-- Registration link -->
                        <div class="form-group my-3">
                            <a href="index.php?page=voters/register">New to this site?</a>
                        </div>
